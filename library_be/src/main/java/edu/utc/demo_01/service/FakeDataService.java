...
import edu.utc.demo_01.dto.patron.FakePatron;
...
//    //endregion
    ...
    public void generateFakeBorrowRecords() {
        ...
        LocalDateTime recentThreshold = today.minusDays(7);
    ...
    public void generateFakeUserWishlist() {
        if (userFavoriteRepository.count() > 1) return;
        List<User> users = userRepository.findAllPatron();
        List<Book> books = bookRepository.findAll();
        for (int i = 0; i < 1500; i++) {
            User user = users.get(random.nextInt(users.size()));
    ...
    private List<Event> createPastEvents(List<User> libraryStaff, int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            Event event = Event.builder()
                    ...
                    .location(location)
                    .createdBy(creator)
                    .maxAttendees(random.nextInt(30, 100))
                    .build();
    ...
    private void createEventParticipants(List<User> allUsers, List<Event> pastEvents, List<Event> upcomingEvents) {
        ...
        for (Event event : pastEvents) {
            ...
            for (User user : participants) {
                ...
                LocalDateTime registrationDate = event.getStartTime().minus(
        ...
        for (Event event : upcomingEvents) {
            ...
            for (User user : participants) {
                ...
                LocalDateTime registrationDate = LocalDateTime.now().minus(
